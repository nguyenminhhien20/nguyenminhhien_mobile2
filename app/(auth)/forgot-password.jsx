...
import { 
  ...
  SafeAreaView, Alert, StatusBar, KeyboardAvoidingView, 
...
export default function ForgotPasswordScreen() {
  ...
  const handleReset = async () => {
    ...
    if (!emailRegex.test(email)) return Alert.alert("Lỗi", "Định dạng Email không hợp lệ.");
    ...
    try {
      ...
      const response = await fetch(`${API_CONFIG.BASE_URL}/api/User/forgot-password`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        const userRes = await fetch(`${API_CONFIG.BASE_URL}/api/User/email/${email}`);
        ...
        Alert.alert("Lỗi", result.message || "Email không tồn tại trong hệ thống.");
      }
    } catch (error) {
        ...
        >
          ...
          <View style={styles.content}>
            ...
            <Text style={styles.subtitle}>
                Đừng lo lắng! Nhập email của bạn để bắt đầu quy trình xác thực và đặt lại mật khẩu.
            </Text>

            <View style={styles.form}>
              <View style={[styles.inputBox, isFocused && styles.inputBoxActive]}>
                <Feather name="mail" size={20} color={isFocused ? "#FFF" : "#444"} />
                <TextInput 
              ...
              <TouchableOpacity 
                style={[styles.mainBtn, (loading || !email) && { opacity: 0.7 }]} 
                onPress={handleReset} 
                disabled={loading}
              >
                {loading ? (
